...
| Phase | テーマ | 期間目安 | 状態 |
|---|---|---|---|
| **Phase 1** | MVP — 写真→OCR→底値ダッシュボード | 4〜6週間 | 🔜 次 |
| **Phase 2** | チラシ対応 + 検索・フィルタ + UX改善 | 3〜4週間 | 未着手 |
| **Phase 3** | Instagram連携 + 底値アラート + 特売通知 | 3〜4週間 | 未着手 |
| **Phase 4** | 自動チラシ収集 + バッチ処理 | 継続的 | 未着手 |
| **Phase 5** | モバイルアプリ化（React Native / Flutter） | 6〜8週間 | 未着手 |
...
写真アップロード → AI による OCR＋構造化抽出 → 底値ダッシュボードの基本フローを構築する。
ユーザがスーパーの店頭で撮影した写真から商品名と価格を自動読み取り、底値を蓄積・表示できる状態をゴールとする。
    ...
    "items": [
      {
        ...
        "category_hint": "推定カテゴリ(酒類/肉類/野菜類/魚介類/卵/乳製品/飲料)",
...
2. **店舗を登録・管理できる**
3. **店頭で撮影した写真をアップロードし、店舗を紐付けできる**
4. **アップロード画像から商品名と価格が自動抽出される**
5. **抽出結果を確認・修正して価格を登録できる**
6. **ダッシュボードで商品別の底値一覧を閲覧できる**
7. **商品の価格推移グラフを閲覧できる**
8. **スマホブラウザで一通りの操作が ログイン→アップロード→確認→ダッシュボード閲覧）ができる**

---
---

# Phase 2: チラシ対応＋検索・フィルタ＋UX改善

## 概要

Phase 1 で構築した基盤にチラシ画像対応を追加し、1枚のチラシから複数商品を一括抽出できるようにする。
商品検索・フィルタの充実、画像ストレージの外部化、商品名寄せの改善でデータ品質を底上げする。
...
### Sprint 5: チラシ画像対応＋画像ストレージ移行（〜1週間）

#### 5-1. チラシ画像の複数商品一括抽出

**Backend:**
- [ ] OCR プロンプトのチラシ対応強化
  - チラシ特有のレイアウト（複数商品が並ぶ）に対応するプロンプト調整
  - 1枚の画像から10〜30商品を抽出するケースへの対応
  - セール価格 vs 通常価格の識別
- [ ] 大きなチラシ画像の分割送信対応
  - 画像を4分割して個別にOCR → 結果をマージ
  - 重複商品の除去ロジック
- [ ] `POST /api/images/{id}/analyze` の改善
  - source_type: "flyer" の場合の処理分岐

**Frontend:**
- [ ] チラシアップロード専用UI
  - source_type 選択（店頭写真 / チラシ）
  - チラシの場合は一括抽出結果をテーブル形式で表示
  - 一括確認・修正UI（スプレッドシート風の編集）
...
| Method | Path | Sprint | 説明 |
|---|---|---|---|
| POST | `/api/images/from-url` | 5 | URL指定で画像取得 |
...
1. **チラシ画像から複数商品を一括抽出・登録できる**
2. **URLを指定してチラシ画像を取り込める**
3. **商品名の表記ブレが自動的に名寄せされる**
4. **カテゴリ別・店舗別・価格帯で商品を検索できる**
5. **画像が Cloudflare R2 に保存される**
6. **スマホのホーム画面に追加できる（PWA）**
...
Instagram のセール投稿スクショからの価格抽出に対応する。
また、底値アラートと特売情報通知機能を実装し、ユーザが能動的に確認しなくても
お買い得情報を受け取れるようにする。
...
### Sprint 8: Instagram スクショ対応＋通知基盤（〜1週間）

#### 8-1. Instagram 投稿スクショの OCR 対応

**Backend:**
- [ ] Instagram スクショ用 OCR プロンプト設計
  - Instagram 特有の UI 要素（いいね数、コメント欄等）を無視するよう指示
  - 投稿テキスト内の価格情報も抽出
  - 画像内のPOPやチラシ部分を識別
- [ ] source_type: "instagram" の処理パス追加
- [ ] Instagramアカウント情報の紐付け（任意）
  - どのスーパーのアカウントの投稿か を店舗と紐付け

**Frontend:**
- [ ] アップロード画面に「Instagram投稿」ソースタイプ追加
- [ ] Instagramスクショ用のOCR結果表示（投稿テキスト + 画像内テキストの統合表示）
...
1. **Instagram 投稿のスクショから商品・価格を抽出できる**
2. **商品をウォッチリストに登録できる**
3. **底値が更新されたらアプリ内通知＋メールで通知される**
4. **お買い得商品（底値+10%以内）が自動判定され通知される**
5. **価格推移グラフで期間切り替え・店舗別表示ができる**
6. **店舗間の価格比較テーブルが表示される**

---
---

# Phase 4: 自動チラシ収集＋バッチ処理
...
| 2 | POST | `/api/images/from-url` | URL指定で画像取得 |
